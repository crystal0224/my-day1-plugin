...
> 정구봉님의 **AI Native Camp - Day 1**을 기반으로, 직무별 커스터마이징과 비개발자 친화 기능을 대폭 강화한 온보딩 스킬입니다.
...
### my-day1 (이 플러그인)
원본의 철학을 계승하되, **완주율 50% → 80%**를 목표로 3가지를 추가했습니다:

#### 1️⃣ **직무별 맞춤 학습** 🎨
원본은 모두에게 같은 설명을 제공했다면, my-day1은:
- **8가지 직무** (HR, 개발자, 디자이너, PM, 분석가, 마케터, 일반 사무, 연구원/학생)
- **150개 직무별 비유**: CLI를 HR은 "Excel 수식", 디자이너는 "Figma 단축키"로 설명
- **63개 직무별 동기부여 스토리**: "매번 똑같은 채용 이메일 보내는 데 지치셨나요?" (HR)
- **직무별 실무 과제**: 배운 걸 내 업무에 바로 적용

#### 2️⃣ **Essential Path (비개발자 완주 시스템)** 🚀
13개 블록을 모두 배우려면 3시간. 바쁜 현업에겐 부담이었습니다:
- **필수 7개만** 완료해도 실무 투입 가능 (1~1.5시간)
- **심화 6개는 선택**: 나중에 필요할 때 돌아와서 배우기
- **마일스톤 배지**: 🎯 기초 → 🏆 핵심 → 🚀 마스터 (성취감 극대화)

#### 3️⃣ **막힘 방지 시스템** 🛟
비개발자는 "명령어가 안 돼요"에서 막혀 이탈합니다:
- **Help 시스템**: "help" 입력하면 4가지 상황별 즉시 도움
- **재진입 플로우**: 중단했다가 다시 와도 이어하기 가능
- **용어 즉시 설명**: 기술 용어 나올 때마다 쉬운 비유로 자동 설명
